 ...
 * La classe permet de gérer la gestion des fichiers
 */
...
public class GestionFichier
{
	...
	public String getFichierCharger() { return this.fichierCharger; }
	 ...
	 */
	public void enregistrer() { this.sauvegarderSommetsArretes(this.getFichierCharger()); }
	 ...
	 * @param path String le fichier passé en paramètre
	 */
	private void sauvegarderSommetsArretes(String path)
	{
		...
		try (PrintWriter writer = new PrintWriter(new FileOutputStream(path, false)))
		{
			...
			{
				sommetLigne = String.format("%-3d, %-4s, %-5d, %-5d, %-10s, %-5d\n", sommet.getId(), sommet.getNom(),sommet.getX(), sommet.getY(), sommet.getCouleur().name(), sommet.getPoint());
			...
			{
				arreteLigne = String.format("%-4d, %-4d, %-5d\n", arrete.getDepart().getId(), arrete.getArrivee().getId(), arrete.getTroncons());
